...
Public consultation/listing PDFs (see `docs/forms/`) are implemented as **lead records** under the Filament "Submissions" navigation group, not as direct writes to canonical tables. Five lead models — `LandPurchaseLead`, `LandSaleLead`, `RentalConsultation`, `BuiltPropertyListingLead`, `PurchaseBuildPropertyLead` — all share a `status` column (`new → in_review → contacted → converted | closed`), a JSON `details` column for long-tail checkbox fields, and `signed_name`/`signed_at`/`ip_address`/`user_agent`/`submitted_at` audit columns. Two more Submissions-group models — `CustomerFeedback` and `PetApplication` — live alongside them but do *not* share the lead-status shape; don't assume the shared columns when touching those.
...
- All `/forms/*` POSTs go through the named `public-form` rate limiter defined in `AppServiceProvider::boot()`: **5 requests/minute, keyed by `IP:path`** — so each form has its own bucket and a user submitting two different forms in quick succession isn't blocked.
- The Policies & Fees text shown above the signature block is loaded from `settings` rows in the `policies` group (keys: `policy.land_purchase`, `policy.land_sale`, `policy.rental_consultation`, `policy.rental_weekly_agent_fees`, `policy.rental_yearly_agent_fees`, `policy.pet_application`, `policy.purchase_build`, `policy.built_property_listing`). Note `policy.purchase_build` — the key drops the `_property` suffix even though the route is `/forms/purchase-build-property`. Edit the text via the Site Settings admin page — no redeploy needed. Defaults are seeded by `database/seeders/FormPolicySeeder.php`.
- The two listing-side resources (`LandSaleLeadResource`, `BuiltPropertyListingLeadResource`) expose a "Convert to Property" row action that creates a draft `Property` record (status `inactive`) and stores the FK back on `converted_property_id`. Always go through this action — don't write to `properties` directly from a lead — because `Property::booted()` price/purpose normalization needs to fire. The purchase-side leads (`LandPurchaseLead`, `PurchaseBuildPropertyLead`) intentionally have no convert action.
...
## Oversight schema (matches the Excel workbook in `docs/xls/`)

The data model mirrors the columns in Revolest's two operational Excel files. See `docs/excel-operations-audit.md` for the full column-by-column mapping and `docs/excel-operations-plan.md` for the implementation phases.

- **Commission flow.** `Owner.commission_percent` is the default rate (default 10%). `Lease.commission_percent_override` wins when set. Use `$owner->commissionRateFor($lease)` rather than reading the column directly; it handles the override fallback. `Payment::booted()` `creating` hook reads this and auto-populates `commission_amount` for `purpose=rent` payments (override-friendly — only fills when blank).
- **Rent cycle advancement.** Every lease has `rent_cycle` (`monthly` / `quarterly` / `annually`) and a derived `next_rent_due_at`. `Lease::booted()` `creating` computes the initial due date from `start_date + cycle`; `Lease::advanceRentDue()` shifts it forward. `Payment::booted()` calls `advanceRentDue()` automatically when status flips to `complete` for a rent payment. Don't manually update `next_rent_due_at` outside this flow.
- **Inspections.** `Inspection` records are the source of truth; `Lease.last_inspection_at` / `last_inspection_status` / `next_inspection_at` are denormalized caches. `Inspection::booted()` syncs these back to the parent Lease on create/update/delete — don't edit those Lease columns directly. New inspection cycle defaults to `inspection_cycle_months` (default 6).
- **Receipt numbers.** `Receipt.receipt_number` follows `RCV-{YEAR}-{6-digit-sequence}`, generated by `Receipt::generateNumber()` in the `creating` hook. Soft-deleted receipts still count for the sequence (so deleting and re-creating doesn't reuse numbers). `Payment::ensureReceipt()` creates a receipt automatically when a payment with `status=complete` is created; do not write to `receipts` manually from controllers.
- **Lead activity timeline.** All 5 lead models (`LandPurchaseLead`, `LandSaleLead`, `RentalConsultation`, `BuiltPropertyListingLead`, `PurchaseBuildPropertyLead`) and `Inquiry` use the `App\Traits\HasLeadActivities` trait, which exposes `$lead->activities()` as a polymorphic `MorphMany` to `lead_activities`. The shared `App\Filament\RelationManagers\LeadActivitiesRelationManager` (one class, used across all 5 lead resources) renders the timeline panel. Use this for follow-up notes, status-change audit, contact attempts — don't bury the history in the model's `notes` text column.
- **Standardized lead columns.** Every lead model now carries `budget_min` / `budget_max` (decimals), `bathrooms` (int), `property_condition`, `intended_use`, `referred_by_name` — the columns from the Excel "Rqst-lst" target sheet. Legacy free-text `budget` field is kept on lead models that had it (LandPurchase, PurchaseBuildProperty) for historical data. Public forms write `referred_by_name` straight from intake.
